import { infoSVG, clearSVG } from "../../utils/svgCollection";
...
export default function ShowInfo() {
  ...
  const toggleInfo = () => {
    ...
    infoRef.current.hasAttribute("open")
  ...
  return (
    <>
            ...
            <div>
              ...
              {pageOne ? (
                <p className="p-5">
                  ...
                  your tasks As a pro, your goal is to speedrun your tasks. When
                  you press START, the first timer will start. When a task is
                  finished on time, the next one will start right away. If you
                  fail a task, the next one won’t start until you close the
                  dialog window about the previous failing. As a setting, you
                  can chose to have a set break between every task (check out
                  settings tab)`
                </p>
              ) : (
      ...
      <button className="inline px-4" onClick={toggleInfo}>
        {infoSVG}
      </button>
    </>
  );
